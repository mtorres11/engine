...
ZBLUR_TEXTURED_VERTEX_PS NormalTexturedVertexVS(TNORMAL_TEXTURED_VERTEX_VS IN)
{
 ...
 float4 vPos = mul(float4(IN.Position, 1.0), g_WorldViewMatrix); 		//vector posicion hacia donde apunta la camara
 ...
 OUT.WorldPosition=mul(float4(IN.Position,1.0),g_WorldMatrix).xyz; 		//valor del pixel <- posicion x,y,z,w del mundo ¿pa qué?
