...
bool CLanguageManager::Init()
{
  ...
  if (parser.LoadFile(m_LanguageXMLPath)) {
    ...
    else
    {
      ...
      {
        const char * l_id = l_Languages(i).GetPszProperty("id");;
        const char * l_path = l_Languages(i).GetPszProperty("path");
        const bool l_defaultLanguage = l_Languages(i).GetBoolProperty("default");
        struct stat stFileInfo;
        int intStat;
        intStat = stat(l_path,&stFileInfo);
        if(intStat == 0)
        {
          LoadXML(l_path);
          if(l_defaultLanguage)
          {
            m_sCurrentLanguage = l_id;
          }
        }
        else
        {
          IsOK = false;
          std::string msg_error = "LanguageManager::LoadXML->Error al intentar leer el archivo de lenguaje. Es posible que un archivo no exista";
          LOGGER->AddNewLog(ELL_ERROR, msg_error.c_str());
          throw CException(__FILE__, __LINE__, msg_error);
        }
      }
